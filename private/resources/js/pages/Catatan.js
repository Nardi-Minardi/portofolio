                    ...
                    {currentData.map((item, index) => {
                        return (
                            <div key={index} className="card" style={{maxHeight:'300px', maxWidth:'350px', marginBottom:'10px'}}>
                                <img style={{padding:'10px', height:'180px', width:'350px', objectFit:'contain'}} src={`${baseURL}public/assets/images/${item.image}`} className="card-img-top" alt="..." />
                                <div className="card-body">
                                    
                                    <p className="card-title" >{item.title}</p>
                                    <a className="btn-swipper" style={{cursor:'pointer', float:'right'}}
                                            href={item.url ? item.url : baseURL+"public/assets/images/"+ item.file} target='_blank'
                                    >lihat</a>
